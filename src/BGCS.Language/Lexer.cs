    ...
    public class Lexer
    {
        ...
        public virtual LexerResult Tokenize(string input, string filename)
        {
            ...
            for (int i = 0; i < length;)
            {
                ...
                {
                    int quoteStart = i;
                    int charStart = quoteStart + 1;

                    if (charStart >= input.Length)
                    {
                        diagnostics.Error("Unexpected end of file.", new(filename, quoteStart, line, column));
                    ...
                    int closingQuote = charEnd + 1;
                    if (closingQuote >= input.Length || input[closingQuote] != '\'')
                    {
                        diagnostics.Error("Invalid token.", new(filename, quoteStart, line, column));
                        return new(diagnostics, null);
                    }

                    int charLength = charEnd - charStart + 1;
                    tokens.Add(new(charStart, charLength, input, new(filename, charStart, line, column), LiteralType.Char));
        ...
        public static bool IsNumber(ReadOnlySpan<char> input, out NumberType type, NumberParseOptions options = NumberParseOptions.AllowAll)
        {
            ...
            if (isHex)
            {
                if (!char.IsAsciiHexDigit(input[0]))
                {
                    return false;
                }
            }
            else if (!char.IsDigit(input[0]) && !(input[0] == '.' && input.Length > 1 && char.IsDigit(input[1])))
            {
                return false;
            }
