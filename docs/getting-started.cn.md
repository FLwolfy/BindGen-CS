...
需要仓库内固定版本时，可以使用标准 .NET tool manifest 安装 `BindGen-CS`。
...
默认输出是 `Generated/Bindings.cs`。`bindgen-cs build` 会把 warning 当 error 编译生成源码；`bindgen-cs diff` 在不替换正式文件的情况下检查已提交 bindings 是否最新。消费项目安装 Runtime：
