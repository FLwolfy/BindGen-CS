...
| 能力 | 状态 | 证据或边界 |
| --- | --- | --- |
| function、enum、constant、typedef | 实机验收 | 五个真实 C API 与 InnoEngine bindings |
...
| target-dependent `long`、`wchar_t`、`long double` | 自动测试 | MSVC/GNU/Darwin ABI mapping；当前实机报告为 Darwin arm64 |
...
| Linux x64 GNU | target model 与跨平台 managed CI；尚无完整真实库/集成报告 |
