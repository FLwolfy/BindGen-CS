...
4. broader C/C++ semantics, adapters, and lifetime contracts (implemented for the declared subset);
...
Status: complete for BGCS. Ten acceptance categories, real-library gates, package/native-consumer tests, and evidence-level documentation are in place. InnoEngine migration is a later phase.
...
Status: complete for the declared desktop subset. `ICppTypeAdapter` and `ICppCallableAdapter` are versioned and deterministic; string/vector/span/array/map/set/optional/variant/expected/path/chrono/smart-pointer lowering uses the same registry, and callables share the naming/exclusion SPI.
...
- Preserve the current adapter contract through reviewed API gates after the first stable release.
- Maintain declared ABI, ownership, and invalidation rules for every built-in adapter.
...
Status: intentionally paused until the three desktop-x64 BGCS reports pass.
...
- Deterministic packages, SPDX SBOM, SLSA provenance, license inventory, vulnerability gate, and GitHub OIDC attestations are implemented.
...
5. Start the InnoEngine migration only after Windows x64, Linux x64, and macOS x64 BGCS reports pass.
