...
For a host without a reviewed snapshot manifest, real-library tests exit nonzero and write candidate SHA-256 files to `artifacts/acceptance/candidate-snapshots/`. CI also uploads the underlying generated files as `snapshot-inputs-<target>`. Review the files and public API before adding the matching target-specific manifests under `tests/real-libraries/`; a candidate is never automatically promoted. Run CI again and require the complete target report. A passing Arm64 baseline cannot stand in for x64 or another operating system.
